...
struct model
{
    ...
    // animmodel copies translate into its first part at load time, so
    // assigning the field alone has no effect once a model is loaded --
    // hence a setter subclasses can keep in sync. Needed by the VR
    // viewmodel, which has to neutralise the hud gun's baked-in
    // screen-space offset (drawhudmodel(), fpsgame/render.cpp).
    virtual void settranslate(const vec &t) { translate = t; }
